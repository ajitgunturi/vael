...
void main() {
  ...
  Future<void> _insertAccount(String id, String familyId,
      {String type = 'savings'}) async {
  ...
  group('BalanceSnapshotDao', () {
    test('insertSnapshot and getByAccount returns snapshots ordered by date',
        () async {
      await _seedFamily('fam_a');
      await _insertAccount('acc_1', 'fam_a');

      await dao.insertSnapshot(BalanceSnapshotsCompanion.insert(
        id: 'snap_1',
        accountId: 'acc_1',
    ...
    test('family isolation — snapshots from other families excluded',
        () async {
      await _seedFamily('fam_a');
