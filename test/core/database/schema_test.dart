...
void main() {
  ...
  group('Accounts table', () {
    ...
    test('should support soft delete via deletedAt', () async {
      ...
      await (db.update(db.accounts)
          ...
          .write(AccountsCompanion(deletedAt: Value(now)));
      ...
      final active = await (db.select(db.accounts)
  ...
  group('Family isolation', () {
    test('should scope queries by familyId', () async {
      ...
      final fam1Cats = await (db.select(db.categories)
            ..where((t) => t.familyId.equals('fam-1')))
          .get();
      expect(fam1Cats, hasLength(2));

      // Query scoped to fam-2
      final fam2Cats = await (db.select(db.categories)
            ..where((t) => t.familyId.equals('fam-2')))
          .get();
